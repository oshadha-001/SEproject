...
public class UserDAO {
    ...
    public boolean updateUserWithPassword(User user) {
        ...
        try {
            conn = DatabaseConnection.getConnection();
            String sql = "UPDATE users SET first_name = ?, last_name = ?, email = ?, phone = ?, role = ?, password_hash = ? WHERE user_id = ?";
            PreparedStatement pstmt = conn.prepareStatement(sql);
            ...
            System.err.println("Error updating user with password: " + e.getMessage());
            return false;
        } finally {
            DatabaseConnection.closeConnection(conn);
        }
